...
local function cmd_twitch_connect()
    ...
    if mod.redeem_twitch_channel_name == nil then
        ...
        Managers.twitch:connect(mod.redeem_twitch_channel_name, callback(mod, "cb_connection_error_callback"), callback(mod, "cb_connection_success_callback"))
...
mod.on_all_mods_loaded = function(status, state_name)
    ...
    if mod.redeem_twitch_channel_name == nil then
        mod:warning("No twitch channel name specified. Use command '/twitch_channel_name [NAME]' to specify the twitch channel.")
    else
